...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // Read earthquake data from the USGS server
        val reader = EarthquakeReader(this)
        reader.run()
    }

    /* The mapView from ArcGIS has functions for the onPause state to run. */
    override fun onPause() {
        super.onPause()
        mapView.pause()
    }

    /* The mapView from ArcGIS has functions for the onResume state to run. */
    override fun onResume() {
        super.onResume()
        mapView.resume()
    }

    /* The mapView from ArcGIS has functions for the onDestroy state to run. */
    override fun onDestroy() {
        super.onDestroy()
        mapView.dispose()
    }

    /* Display all the earthquakes received.  This uses ArcGIS library calls.
       All of the functions are thread safe with respect to UI vs Background
       threads in Android.
     */
